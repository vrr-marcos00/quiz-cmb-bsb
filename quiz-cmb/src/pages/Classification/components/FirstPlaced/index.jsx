...
function FisrtPlaced({ classification }) {
  ...
  return (
    <div className="first-placed-row">
      ...
      {sortPlayers.map((player, index) => {
        return (
          <div
            className={`first-placed ${addClassThreeFirsts[index]}`}
            key={index}
          >
            ...
            <div className="first-placed-college">
              <img
                src={images_colleges[player.studentId].image}
                alt={player.studentId}
              />
              <span>{player.studentId}</span>
            </div>
